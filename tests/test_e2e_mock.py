...
def test_check_signal_bus_unread():
    ...
    with patch("signal_parser._check_signal_legacy", return_value=True) as m:
...
def test_check_signal_bus_empty():
    """旧格式无未读返回 False。"""
    from launcher import check_signal
    with patch("signal_parser._check_signal_legacy", return_value=False):
...
def test_new_format_bus_dispatch():
    """type=bus → signal_parser._check_bus。"""
    from launcher import check_signal
    with patch("signal_parser._check_bus", return_value=True) as m:
        sig = {"type": "bus", "spec": {"category": "security"}, "filter": "", "timeout_sec": 10}
        assert check_signal(sig) is True
        m.assert_called_once_with({"category": "security"}, "", 10)


def test_new_format_shell_dispatch():
    """type=shell → signal_parser._check_shell。"""
    from launcher import check_signal
    with patch("signal_parser._check_shell", return_value=False) as m:
        sig = {"type": "shell", "spec": {"command": "echo ok"}, "filter": "ok", "timeout_sec": 5}
        assert check_signal(sig) is False
        m.assert_called_once_with({"command": "echo ok"}, "ok", 5)


def test_new_format_http_dispatch():
    """type=http → signal_parser._check_http。"""
    from launcher import check_signal
    with patch("signal_parser._check_http", return_value=True) as m:
        sig = {"type": "http", "spec": {"url": "http://localhost:8890"}, "filter": ""}
        assert check_signal(sig) is True
        m.assert_called_once_with({"url": "http://localhost:8890"}, "", 10)


def test_new_format_journalctl_dispatch():
    """type=journalctl → signal_parser._check_journalctl。"""
    from launcher import check_signal
    with patch("signal_parser._check_journalctl", return_value=False) as m:
        sig = {"type": "journalctl", "spec": {"unit": "sister-agent-dkk"}, "filter": "ERROR"}
        assert check_signal(sig) is False
        m.assert_called_once_with({"unit": "sister-agent-dkk"}, "ERROR", 10)


def test_new_format_custom_dispatch():
    """type=custom → signal_parser._check_custom。"""
    from launcher import check_signal
    with patch("signal_parser._check_custom", return_value=True) as m:
        sig = {"type": "custom", "spec": {"description": "manual trigger"}, "filter": "trigger"}
        assert check_signal(sig) is True
        m.assert_called_once_with({"description": "manual trigger"}, "trigger")
