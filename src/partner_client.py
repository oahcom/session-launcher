...
from sentinel import read_sentinel, SENTINEL_DIR
from core import (
    output as ccs_capture_output,
    check_wake_permission, wake_ccs, force_start_ccs,
    _find_claude_pid, send, BUS_CLIENT,
)
...
class PartnerClient:
    ...
    def confirm_delivery(self, task_id: str, target_role: str,
        ...
        while (elapsed := time.time() - start_ts) < timeout:
            ...
            if task and task.get("status") != "created":
                return {
                    "confirmed": True,
                    "task": task,
    ...
    def resolve(self, role: str) -> dict:
        ...
        if alive:
            ...
            pid = _find_claude_pid(tmux_name) or pid
        ...
        if alive:
            try:
                output = ccs_capture_output(role, tail=1)
                if output:
    ...
    def check_wake_permission(self, target: str) -> bool:
        """检查本角色是否有权唤醒 target。"""
        return check_wake_permission(self.role, target)
    ...
    def wake(self, role: str, context: str = "",
        ...
        return wake_ccs(role, context=context, by_role=self.role if not force else "")
