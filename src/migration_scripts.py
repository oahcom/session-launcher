...
if __name__ == "__main__":
    ...
    if cmd == "pre-flight":
        ...
        for s in r:
            status = "✅" if s["status"] == "pending" else "⏭️"
            print(f"{status} [{s['table']}] {s['action']}")

    elif cmd == "export":
        path = export_backup()
        print(f"✅ 已导出到: {path} ({path.stat().st_size} bytes)")
        ...
        if dry:
            ...
            print(f"✅ 已清空 {r.get('truncated_instances', 0)} 个实例，备份到 {r.get('backup_path')}")

    elif cmd == "restore":
        if len(sys.argv) < 3:
            print("用法: python3 migration_scripts.py restore <backup.jsonl>")
            sys.exit(1)
        ...
        print(f"未知命令: {cmd}")
        print("可用: pre-flight | dry-run | export | truncate | restore | migrate")
        sys.exit(1)
